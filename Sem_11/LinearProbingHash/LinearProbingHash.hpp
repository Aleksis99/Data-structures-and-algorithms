...
public:
	...
	bool find(int key, const ValueType &output) const override;
...
template <typename ValueType>
bool LinearProbingHash<ValueType>::find(int key, const ValueType &output) const
{
	size_t hashCode = Hash<ValueType>::hashFunc->calculateHashCode(key) % linearProb.capacity();
	...
	while (!linearProb[hashCode].empty || linearProb[hashCode].hasBeenDeleted)
	{
		...
		{
			output = linearProb[hashCode].value;
			return true;
		}
		(++hashCode) % linearProb.capacity();
	}
